...
// Коментар може займати декілька рядків, тому парситься окремо:
// беремо все після мітки "Коментар:" аж до порожнього рядка,
// до мітки "Відправлено" або до кінця повідомлення.
function extractComment(text) {
  const regex = /Коментар\s*:\s*([\s\S]+?)(?:\n\s*\n|\n\s*🕒|\n\s*Відправлено|$)/iu;
  const match = text.match(regex);
  return match && match[1] ? match[1].trim() : null;
}
